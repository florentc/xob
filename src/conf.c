...
static int config_setting_lookup_dim(const config_setting_t* setting, const char* name, dim_t* value)
{
    double rel;
    int abs;
    int success_status;
    ...
    if (dim_setting != NULL)
    {
        if (config_setting_lookup_float(dim_setting, "relative", &rel))
        {
            ...
            else
            {
                fprintf(
                        stderr,
                        "Error: in configuration, line %d - Out of range relative value.\n",
        ...
        else
        {
                fprintf(
                        stderr,
                        "Error: in configuration, line %d - Invalid (maybe a missing dot '.') or missing relative value.\n",
                        config_setting_source_line(dim_setting)
                       );
        }
    }
...
static int config_setting_lookup_color(const config_setting_t* setting, const char* name, const char** value)
{
    ...
    if (config_setting_lookup_string(setting, name, &colorstring))
    {
        ...
        else
        {
            fprintf(
                    stderr,
                    "Error: in configuration, line %d - Invalid color specification.\n",
...
static int config_setting_lookup_colorspec(
        ...
        colorspec_t* value)
{
    ...
    if (colorspec_setting != NULL)
    {
        ...
        config_setting_lookup_color(colorspec_setting, "border", &(value -> border));
    }

    return success_status;
}

static int config_setting_lookup_overflowmode(
        const config_setting_t* setting,
        const char* name,
        overflow_mode_t* value)
{
    ...
    if (config_setting_lookup_string(setting, name, &stringvalue))
    {
        ...
        else
        {
            fprintf(
                    stderr,
                    "Error: in configuration, line %d - Invalid overflow mode.\n",
                    config_setting_source_line(setting)
                   );
        }
    }
        ...
        style_t default_style)
{
    ...
    if (config_read(&config, file))
    {
        ...
        {
            config_setting_lookup_int(style_config, "thickness", &style.thickness);
            ...
            config_setting_lookup_overflowmode(style_config, "overflow", &style.overflow);
            color_config = config_setting_get_member(style_config, "color");
            if (color_config != NULL)
            {
                config_setting_lookup_colorspec(color_config, "normal", &style.color.normal);
                config_setting_lookup_colorspec(color_config, "overflow", &style.color.overflow);
                config_setting_lookup_colorspec(color_config, "alt", &style.color.alt);
                config_setting_lookup_colorspec(color_config, "altoverflow", &style.color.altoverflow);
            }
        }
        else
        {
    ...
    else
    {
        fprintf(
                stderr,
                "Error: in configuration, line %d - %s\n",
